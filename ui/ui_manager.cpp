...
namespace hbase
{
	namespace menu_press_timer
	{
		...
		bool IsButtonTapped(const menu_press_timer::Button& button)
		{
			...
			switch (button)
			{
			case Button::Right: isTapped = g_keyBoard->KeyJustUp(VK_NUMPAD6); break;
			case Button::Left: isTapped = g_keyBoard->KeyJustUp(VK_NUMPAD4); break;
			case Button::Down: isTapped = g_keyBoard->KeyJustUp(VK_NUMPAD2); break;
			case Button::Up: isTapped = g_keyBoard->KeyJustUp(VK_NUMPAD8); break;
			case Button::Back: isTapped = g_keyBoard->KeyJustUp(VK_NUMPAD0); break;
			case Button::Accept: isTapped = g_keyBoard->KeyJustUp(VK_NUMPAD5); break;
			case Button::num7: isTapped = g_keyBoard->KeyJustUp(VK_NUMPAD7); break;
			case Button::num1: isTapped = g_keyBoard->KeyJustUp(VK_NUMPAD1); break;
			}
			return isTapped;
		}
	...
	void ui_mgr::inputBox()
	{
		if (m_isInput)
		{
			...
			list->AddText(ImVec2(m_pos_x, m_pos_y), m_optionTextColor, " ‰»Îƒ⁄»›(»∑»œº¸Õ£÷π ‰»Î ÕÀ∏Òº¸…æ≥˝◊Ó∫Û◊÷∑˚):");
	...
	{
		if (g_keyBoard->KeyJustUp(VK_MULTIPLY))
		{
			m_opened ^= true;
		}
	...
	void ui_mgr::drawHeader()
	{
		...
		if (g_ui_settings.options["header"]["count"].get<int>() > 1)
		{
			...
			list->AddImage((ImTextureID)g_renderer->m_header_gif_texture.at(idleFrame), ImVec2(m_pos_x, m_pos_y), ImVec2(m_pos_x + m_menu_width, m_pos_y + m_header_height), ImVec2(0, 0), ImVec2(1, 1), m_header_color);
		}
		else
		{
			list->AddImage((ImTextureID)g_renderer->m_header_texture, ImVec2(m_pos_x, m_pos_y), ImVec2(m_pos_x + m_menu_width, m_pos_y + m_header_height), ImVec2(0, 0), ImVec2(1, 1), m_header_color);
		}
	...
	void ui_mgr::drawSubMenuBar(AbstractSubmenu* sub)
	{
		...
		list->AddRectFilled(ImVec2(m_pos_x, m_draw_cursor_y), ImVec2(m_pos_x + m_menu_width, m_draw_cursor_y + m_option_height), m_submenubar_color);
		char ligfttText[64] = {};
		sprintf_s(ligfttText, "HOME->%s", sub->GetName());

		list->AddText(ImVec2(m_pos_x, m_draw_cursor_y), m_optionTextColor, ligfttText);
		...
		list->AddText(ImVec2(optionPosX, m_draw_cursor_y), m_optionTextColor, rightText);
	...
	void ui_mgr::drawOption(AbstractOption* opt, bool selected)
	{
		...
		list->AddText(ImVec2(optionPosX, m_draw_cursor_y), selected ? text_rgba_active : text_rgba, text);
	...
	void ui_mgr::drawFooter()
	{
		...
		list->AddRectFilled(ImVec2(m_pos_x, m_draw_cursor_y), ImVec2(m_pos_x + m_menu_width, m_draw_cursor_y + m_option_height), m_footerColor);
		m_draw_cursor_y += m_option_height;
	}
	...
	void ui_mgr::drawDescription()
	{
		...
		m_draw_cursor_y += p;
		ImDrawList* list = ImGui::GetForegroundDrawList();
		list->AddRectFilled(ImVec2(m_pos_x, m_draw_cursor_y + p), ImVec2(m_pos_x + m_menu_width, m_draw_cursor_y + m_option_height + p), m_description_color);

		list->AddText(ImVec2(m_pos_x, m_draw_cursor_y + p), m_optionTextColor, description);
		m_draw_cursor_y += m_option_height;
	}
